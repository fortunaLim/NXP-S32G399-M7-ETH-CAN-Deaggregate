...
#define FTM_GPT_IP_SW_MAJOR_VERSION_C             4
#define FTM_GPT_IP_SW_MINOR_VERSION_C             0
#define FTM_GPT_IP_SW_PATCH_VERSION_C             2
...
#endif
static inline void Ftm_Gpt_Ip_SetChannelEnableInterrupt(uint8 instance, uint8 channel, boolean enable);
 ...
 * @param[in]   instance    FTM hw instance number
 * @param[in]   channel     FTM hw channel number
 * @param[in]   enable - Enables the FTM channel(n) interrupt
 *              - True : Enable the FTM channel(n) interrupt.
 *              - False: Disable the FTM channel(n) interrupt.
 */
static inline void Ftm_Gpt_Ip_SetChannelEnableInterrupt(uint8 instance, uint8 channel, boolean enable)
{
    ftmGptBase[instance]-> CONTROLS[channel].CSC = (ftmGptBase[instance]-> CONTROLS[channel].CSC & ~(FTM_CSC_CHIE_MASK)) | FTM_CSC_CHIE(enable ? 1UL : 0UL);
}
