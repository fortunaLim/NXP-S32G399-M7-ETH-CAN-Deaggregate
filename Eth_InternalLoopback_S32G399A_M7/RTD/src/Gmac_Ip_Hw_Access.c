...
#define GMAC_IP_HW_ACCESS_SW_PATCH_VERSION_C               2
...
void GMAC_CommonIRQHandler(uint32 Instance)
{
    ...
    for (i = 0; i < Gmac_apxState[Instance]->RxRingCount; i++)
    {
    ...
    for (i = 0; i < Gmac_apxState[Instance]->TxRingCount; i++)
    {
        if ((Base->DMA_INTERRUPT_STATUS & (1UL << (uint32)i)) != 0U)
        {
            Gmac_Ip_ChannelType *ChBase = Gmac_apxChBases[Instance][i];
                             ...
                             uint32 Crc)
{
    ...
    if (GMAC_MAC_VLAN_TAG_DATA_ETV_MASK != (Base->MAC_VLAN_TAG_DATA & GMAC_MAC_VLAN_TAG_DATA_ETV_MASK))
    {
        Index = Index - 1UL;
    }
...
#if defined(ERR_IPV_GMAC_E050176)
    QueueBase->MTL_RXQ_OPERATION_MODE |= GMAC_MTL_RXQ0_OPERATION_MODE_RSF_MASK;
#endif
}
...
void GMAC_SetVlanTagInvMatch(GMAC_Type * Base, boolean Enable)
{
    if (Enable)
    {
        Base->MAC_VLAN_TAG_CTRL |= GMAC_MAC_VLAN_TAG_CTRL_VTIM_MASK;
    }
    else
    {
        Base->MAC_VLAN_TAG_CTRL &= ~GMAC_MAC_VLAN_TAG_CTRL_VTIM_MASK;
                                ...
                                boolean En12bitMatch)
{
    if (EnInnerVlanMatch)
    {
        Base->MAC_VLAN_TAG_DATA |= GMAC_MAC_VLAN_TAG_DATA_ERIVLT_MASK;
    }
    else
    {
        Base->MAC_VLAN_TAG_DATA &= ~GMAC_MAC_VLAN_TAG_DATA_ERIVLT_MASK;
    ...
    if (EnSvlanMatch)
    {
        ...
        Base->MAC_VLAN_TAG_DATA |= GMAC_MAC_VLAN_TAG_DATA_ERSVLM_MASK;
#else
        /*This to be default for this errata*/
        Base->MAC_VLAN_TAG_DATA &= ~GMAC_MAC_VLAN_TAG_DATA_ERSVLM_MASK;
#endif
    }
    else
    ...
    {
#ifndef ERR_IPV_GMAC_E050109
        Base->MAC_VLAN_TAG_DATA |= GMAC_MAC_VLAN_TAG_DATA_DOVLTC_MASK;
#else
        /*This to be default for this errata*/
        Base->MAC_VLAN_TAG_DATA &= ~GMAC_MAC_VLAN_TAG_DATA_DOVLTC_MASK;
#endif
    ...
    {
#ifndef ERR_IPV_GMAC_E050109
        Base->MAC_VLAN_TAG_DATA |= GMAC_MAC_VLAN_TAG_DATA_ETV_MASK;
#else
        /*This to be default for this errata*/
        Base->MAC_VLAN_TAG_DATA &= ~GMAC_MAC_VLAN_TAG_DATA_ETV_MASK;
#endif
    ...
    Base->MAC_VLAN_TAG_DATA |= GMAC_MAC_VLAN_TAG_DATA_VEN_MASK;
}
/*FUNCTION**********************************************************************
 *
                                            ...
                                            uint16 * VlanTag)
{
    ...
    {
        /* Read the VLAN tag. */
        *VlanTag = (uint16)(Base->MAC_VLAN_TAG_DATA &
