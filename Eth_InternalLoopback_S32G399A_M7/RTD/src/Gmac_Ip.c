                                      ...
                                      )
{
    ...
    if (((Bd->Des1 & GMAC_RDES1_TSA_MASK) != 0U) && ((Bd->Des3 & GMAC_RDES3_LD_MASK) != 0U))
    {
        ...
        if ((uint32)Gmac_apxState[Instance]->RxCurrentDesc[Ring] >= (uint32)&ListBd[ChBase->DMA_RXDESC_RING_LENGTH + 1UL])
        {
                                       ...
                                       const Gmac_CtrlConfigType *Config)
{
    ...
    Config->Gmac_pCtrlState->EnableMDIO = FALSE;
                            ...
                            )
{
    ...
    for (i = 0U; i < Config->TxRingCount; i++)
    {
        ...
        {
            Gmac_apxChBases[Instance][i]->DMA_CONTROL = GMAC_DMA_CH0_CONTROL_DSL(8U / FEATURE_GMAC_DATA_BUS_WIDTH_BYTES) | GMAC_DMA_CH0_CONTROL_PBLx8(1);
        ...
        {
            Gmac_apxChBases[Instance][i]->DMA_CONTROL = GMAC_DMA_CH0_CONTROL_DSL(8U / FEATURE_GMAC_DATA_BUS_WIDTH_BYTES);
    ...
    for (i = 0; i <= ChBase->DMA_TXDESC_RING_LENGTH; i++)
    {
        if (Bd->Info0 == ((uint32)Buff->Data))
        {
            ...
            else
            {
                ...
                /* Restore Buffer address pointer and clear the descriptor after the Status was read. */
                Bd->Des0  = Bd->Info0;
                Bd->Info0 = 0U;
                Bd->Info1 &= ~GMAC_INFO1_LOCKED_MASK;
            }
                         ...
                         uint32 ModuleClk)
{
    ...
    Gmac_apxState[Instance]->EnableMDIO = TRUE;
}
                                    ...
                                    uint32 TimeoutMs)
{
    ...
    if (Gmac_apxState[Instance]->EnableMDIO == (boolean)TRUE)
    {
                         ...
                         const Gmac_Ip_VlanConfigType * VlanConfig)
{
    ...
    Base->MAC_VLAN_TAG_CTRL = GMAC_MAC_VLAN_TAG_CTRL_EDVLP((VlanConfig->EnDoubleVlan) ? 1UL : 0UL) |
    ...
    Base->MAC_VLAN_INCL = GMAC_MAC_VLAN_INCL_VLTI(0U) |
    ...
    {
        Base->MAC_VLAN_INCL |= GMAC_MAC_VLAN_INCL_CBTI_MASK;
    }
                                                         ...
                                                         uint16 VlanTag)
{
    ...
    if (((uint32)((Base->MAC_VLAN_INCL & GMAC_MAC_VLAN_INCL_VLC_MASK) >>
        GMAC_MAC_VLAN_INCL_VLC_SHIFT)) ==
        ((uint32)GMAC_VLAN_TAG_INSERTION))
    {
        ...
        if ((Base->MAC_VLAN_INCL & GMAC_MAC_VLAN_INCL_BUSY_MASK) != 0U)
        {
            Status =  GMAC_STATUS_BUSY;
        }
        else
        {
            Base->MAC_VLAN_INCL &= ~GMAC_MAC_VLAN_INCL_ADDR_MASK;

            /* Initiate indirect write access for the selected Ring. */
            Base->MAC_VLAN_INCL |= (uint32)(((uint32)Ring) <<
                                   GMAC_MAC_VLAN_INCL_ADDR_SHIFT);

            /* Enable write access. */
            Base->MAC_VLAN_INCL |= GMAC_MAC_VLAN_INCL_RDWR(1U);

            Base->MAC_VLAN_INCL &= ~(GMAC_MAC_VLAN_INCL_CSVL_MASK |
                                     GMAC_MAC_VLAN_INCL_VLT_MASK);

            /* Set the VLAN type and tag to be inserted/replaced. */
            Base->MAC_VLAN_INCL |= ((uint32)(((uint32)VlanType) <<
                                    GMAC_MAC_VLAN_INCL_CSVL_SHIFT)) |
                                   ((uint32)(((uint32)VlanTag) <<
                                    GMAC_MAC_VLAN_INCL_VLT_SHIFT));

            /* Disable write access. */
            Base->MAC_VLAN_INCL &= ~GMAC_MAC_VLAN_INCL_RDWR_MASK;
            /* Add fault label for testing */
            #ifdef MCAL_ENABLE_FAULT_INJECTION
...
#ifndef ERR_IPV_GMAC_E050109
        Base->MAC_VLAN_TAG_CTRL |= GMAC_MAC_VLAN_TAG_CTRL_VTHM_MASK;
#endif
                                                ...
                                                uint16 VlanTag)
{
    ...
    /* Clear VLAN tag. */
    Base->MAC_VLAN_TAG_DATA &= ~GMAC_MAC_VLAN_TAG_DATA_VID_MASK;
    /* Clear filter offset. */
    Base->MAC_VLAN_TAG_CTRL &= ~GMAC_MAC_VLAN_TAG_CTRL_OFS_MASK;

    /* Set the VLAN tag. */
    Base->MAC_VLAN_TAG_DATA |= ((uint32)(((uint32)VlanTag) <<
                               GMAC_MAC_VLAN_TAG_DATA_VID_SHIFT)) |
                               GMAC_MAC_VLAN_TAG_DATA_VEN_MASK;

    /* Set filter offset. */
    Base->MAC_VLAN_TAG_CTRL |= (uint32)(((uint32)FilterIdx) <<
                               GMAC_MAC_VLAN_TAG_CTRL_OFS_SHIFT);
    /* Enable write operation. */
    Base->MAC_VLAN_TAG_CTRL &= ~GMAC_MAC_VLAN_TAG_CTRL_CT_MASK;
    /* Set busy bit. */
    Base->MAC_VLAN_TAG_CTRL |= GMAC_MAC_VLAN_TAG_CTRL_OB_MASK;
    /* Add fault label for testing */
    #ifdef MCAL_ENABLE_FAULT_INJECTION
    ...
    do
    {
        if ((Base->MAC_VLAN_TAG_CTRL & GMAC_MAC_VLAN_TAG_CTRL_OB_MASK) == 0U)
        {
            Status = GMAC_STATUS_SUCCESS;
            break;
        }
                         ...
                         const Gmac_Ip_SysTimeConfigType * SysTimeConfig)
{
    ...
    Base->MAC_LOG_MESSAGE_INTERVAL = 0xfb;// fd=8 / fc=16 / fb=32 / fa=64 / f9=128
